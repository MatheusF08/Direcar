...
function App( ) {
  ...
  useEffect(() => { if (isAuthenticated) { setShowRegister(false); fetchEstimates(); } }, [isAuthenticated]);
  const handleLoginSuccess = () => setIsAuthenticated(true);
  const handleLogout = () => { localStorage.removeItem('authToken'); setIsAuthenticated(false); setEstimates([]); };
  const handleViewEstimate = (estimate: Estimate) => { setSelectedEstimate(estimate); setEditedEstimate(estimate); };
  const handleSaveEstimate = async () => {
    const token = localStorage.getItem('authToken');
    if (!token || !editedEstimate) return;
    const updateData = { status: editedEstimate.status };
    try {
      const response = await fetch(`${API_URL}/api/estimates/${editedEstimate.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(updateData), });
      if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.message || 'Falha ao salvar as alterações.'); }
    ...
    } catch (err: any) { console.error("Erro ao salvar orçamento:", err); alert(`Erro: ${err.message}`); }
  };
  const handleDeleteEstimate = async (id: number) => {
    ...
    if (!token) return;
    try {
      const response = await fetch(`${API_URL}/api/estimates/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` }, });
      if (!response.ok) throw new Error('Falha ao deletar o orçamento.');
      setEstimates(prev => prev.filter(est => est.id !== id));
      alert('Orçamento deletado com sucesso!');
    } catch (err: any) { console.error("Erro ao deletar orçamento:", err); alert(`Erro: ${err.message}`); }
  };
  const handleGeneratePdf = (estimate: Estimate) => {
    ...
    } catch (error) { console.error('Erro ao gerar PDF:', error); alert('Ocorreu um erro ao tentar gerar o PDF.'); }
  };
  const renderContent = () => {
    if (isAuthenticated) {
      return (<> <header className="app-header"><h1>Direcar - Gestão de Orçamentos</h1><button onClick={handleLogout} className="logout-button">Sair</button></header> <main> <EstimateForm onEstimateCreated={fetchEstimates} /> <VehicleList estimates={estimates} onDelete={handleDeleteEstimate} onView={handleViewEstimate} onGeneratePdf={handleGeneratePdf} /> </main> {selectedEstimate && editedEstimate && (<div className="modal-overlay"><div className="modal-content"><div className="modal-header"><h2>Editar Orçamento Nº {String(selectedEstimate.id).padStart(4, '0')}</h2><button className="close-button" onClick={() => setSelectedEstimate(null)}>×</button></div><div className="modal-body"><p><strong>Cliente:</strong> {selectedEstimate.vehicle.clientName}</p><p><strong>Veículo:</strong> {`${selectedEstimate.vehicle.brand} ${selectedEstimate.vehicle.model} (${selectedEstimate.vehicle.plate})`}</p><p><strong>Status Atual:</strong> {selectedEstimate.status.replace('_', ' ')}</p><hr /><h4>Alterar Status para:</h4><div className="status-change-buttons"><button className="action-button ag-aprovacao" onClick={() => setEditedEstimate(prev => prev ? { ...prev, status: 'AG_APROVACAO' } : null)}>Ag. Aprovação</button><button className="action-button aprovado" onClick={() => setEditedEstimate(prev => prev ? { ...prev, status: 'APROVADO' } : null)}>Aprovado</button><button className="action-button finalizado" onClick={() => setEditedEstimate(prev => prev ? { ...prev, status: 'FINALIZADO' } : null)}>Finalizado</button><button className="action-button cancelado" onClick={() => setEditedEstimate(prev => prev ? { ...prev, status: 'CANCELADO' } : null)}>Cancelado</button></div><p className="new-status-preview"><strong>Novo Status:</strong> {editedEstimate.status.replace('_', ' ')}</p></div><div className="modal-footer"><button className="cancel-button" onClick={() => setSelectedEstimate(null)}>Cancelar</button><button className="save-button" onClick={handleSaveEstimate}>Salvar Alterações</button></div></div></div>)} </>);
  ...
  return (<div className={isAuthenticated ? 'app-container' : 'login-page'}>{renderContent()}</div>);
}
export default App;
