...
function App( ) {
  ...
  const handleGeneratePdf = (estimate: Estimate) => {
    try {
      ...
      doc.text('Orçamento Automotivo', 14, 26);
      doc.text(`Orçamento Nº: ${String(id).padStart(4, '0')}`, 140, 20);
      ...
      {isAuthenticated ? (
        <>
          <header className="app-header">
            <h1>Direcar - Gestão de Orçamentos</h1>
            ...
            <div className="modal-overlay">
              <div className="modal-content">
                <div className="modal-header">
                  <h2>Editar Orçamento Nº {String(selectedEstimate.id).padStart(4, '0')}</h2>
                  <button className="close-button" onClick={() => setSelectedEstimate(null)}>×</button>
                </div>
                <div className="modal-body">
                  ...
                  <p><strong>Status Atual:</strong> {selectedEstimate.status.replace('_', ' ')}</p>
                  <hr />
